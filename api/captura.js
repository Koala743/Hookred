...
// ── UUID DETECTION ────────────────────────────────────────────────
// Detecta si la URL contiene un UUID (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx)
// y termina en .ts — indica un fragmento TS de video XxAzul
...
function esInteresante(item) {
  ...
  if (item.video_uuid)                                       return true;  // XxAzul siempre es interesante
  return false;
}
...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    // ── Detección XxAzul: UUID en URL con .ts ─────────────────────
    ...
    const item = {
      ...
      media_tipo,
      video_uuid,   // null si no es XxAzul, UUID string si lo es
      count:       1,
    };
    ...
    return res.status(200).json({
      ...
      seq: item.seq,
      last_seq: global._seq,
      video_uuid,   // lo devolvemos para que la extensión sepa si es XxAzul
    });
  }
  ...
  if (req.method === "GET") {
    const {
      ...
      media, importante, q,
      video_uuid,             // filtrar por UUID específico de XxAzul
      xxazul,                 // "true" → solo items con video_uuid
      limit = 100, offset = 0,
      orden = "seq_desc",
    } = req.query;
    ...
    // ── Resumen de grupos XxAzul (UUIDs únicos y cantidad de segs) ─
    const xxazulResumen = {};
    global._capturas.forEach(i => {
      if (!i.video_uuid) return;
      if (!xxazulResumen[i.video_uuid]) xxazulResumen[i.video_uuid] = 0;
      xxazulResumen[i.video_uuid]++;
    });
    ...
    return res.status(200).json({
      ...
      session_id: global._sessionId,
      last_seq: global._seq,
      xxazul_grupos: xxazulResumen,   // { uuid: cantSegs, ... }
      items,
    });
  }
